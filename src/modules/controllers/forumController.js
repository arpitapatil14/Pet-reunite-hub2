...
module.exports.getBoard = async (req, res, next) => {
  try {
    ...
    if (name) {
      ...
      return next(createError.BadRequest('Pet Category or boardId must not be empty'))
    }
...
module.exports.createBoard = async (req, res, next) => {
  try {
    ...
    if (name.trim() === '') return next(createError.BadRequest(' Pet Category name must not be empty'))
    if (title.trim() === '') return next(createError.BadRequest('Pet title must not be empty'))
    ...
    if (nameExist) return next(createError.Conflict('Category with this short name is already been created'))
